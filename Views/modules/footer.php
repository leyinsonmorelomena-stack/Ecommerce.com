<div class="container-fluid bg-dark small">
  <div class="container py-5 text-light">
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3">

    <!-- primera columna -->
<div class="col row">
  <!-- 🔌 Accesorios de Computo -->
  <div class="col-12 col-sm-4 col-md-3 col-lg-4">
    <h4 class="lead">
      <a href="#" class="text-uppercase"></a>Accesorios de Cómputo
    </h4>

    <hr class="border-white">

    <ul>
      <li>
        <a href="#">-Teclados mecánicos e inalámbricos</a>
      </li>

      <li>
        <a href="#">-Mouse ergonómicos / gamer / inalámbricos</a>
      </li>

      <li>
        <a href="#">-Alfombrillas con carga inalámbrica</a>
      </li>

      <li>
        <a href="#">-Monitores adicionales (curvos, 4K, etc.)</a>
      </li>

      <li>
        <a href="#">-Docking stations / hubs USB-C</a>
      </li>
    </ul>
  </div>

  <!-- 📱 Accesorios para Dispositivos Móviles -->
  <div class="col-12 col-sm-4 col-md-3 col-lg-4">
    <h4 class="lead">
      <a href="#" class="text-uppercase"></a>Accesorios para Dispositivos Móviles
    </h4>

    <hr class="border-white">

    <ul>
      <li>
        <a href="#">-Fundas protectoras para celulares y tablets</a>
      </li>

      <li>
        <a href="#">Vidrios templados</a>
      </li>

      <li>
        <a href="#">-Cargadores rápidos y cables</a>
      </li>

      <li>
        <a href="#">-Power banks / baterías externas</a>
      </li>

      <li>
        <a href="#">-Cargadores inalámbricos</a>
      </li>
    </ul>
  </div>

  <!-- 🎧 Audio y Multimedia -->
  <div class="col-12 col-sm-4 col-md-3 col-lg-4">
    <h4 class="lead">
      <a href="#" class="text-uppercase"></a>Audio y Multimedia
    </h4>

    <hr class="border-white">

    <ul>
      <li>
        <a href="#">-Bocinas Bluetooth</a>
      </li>

      <li>
        <a href="#">-Audífonos inalámbricos (AirPods, over-ear, in-ear)</a>
      </li>

      <li>
        <a href="#">-Micrófonos de condensador / streaming</a>
      </li>

      <li>
        <a href="#">-Interfaces de audio USB</a>
      </li>

      <li>
        <a href="#">-Luces LED RGB para setup</a>
      </li>
    </ul>
  </div>

  <!-- 📡 Conectividad y Redes -->
  <div class="col-12 col-sm-4 col-md-3 col-lg-4">
    <h4 class="lead">
      <a href="#" class="text-uppercase"></a>Conectividad y Redes
    </h4>

    <hr class="border-white">

    <ul>
      <li>
        <a href="#">-Routers y repetidores WiFi</a>
      </li>
  
      <li>
        <a href="#">-Switches de red</a>
      </li>

      <li>
        <a href="#">-Adaptadores de red USB</a>
      </li>

      <li>
        <a href="#">-Antenas WiFi</a>
      </li>

      <li>
        <a href="#">-Discos duros externos y SSDs portátiles</a>
      </li>
    </ul>
  </div>

  <!-- 🖥️ Accesorios de Oficina Tecnológica -->
  <div class="col-12 col-sm-4 col-md-3 col-lg-4">
    <h4 class="lead">
      <a href="#" class="text-uppercase"></a>Accesorios de Oficina Tecnológica
    </h4>

    <hr class="border-white">

    <ul>
      <li>
        <a href="#">-Sillas ergonómicas gamer/office</a>
      </li>

      <li>
        <a href="#">-Bases refrigerantes para laptops</a>
      </li>

      <li>
        <a href="#">-Organizadores de cables</a>
      </li>

      <li>
        <a href="#">-Estaciones de carga múltiples</a>
      </li>

      <li>
        <a href="#">-Proyectores portátiles</a>
      </li>
    </ul>
  </div>
</div>


    <!-- segundo columna -->
     <div class="col my-3 my-lg-0 px-lg-5 text-light">

     <h1 class="lead small">Dudas E Inquietudes, Contactenos En:</h1>
     <br>

     <h1 class="lead small">
      <i class="fa fa-envelope pe-2"></i>(304) 673-49-89
        <br><br>

        <i class="fa fa-map-marker pe-2"></i>Cra. 51 # 53-22 - La Candelaria, Medellín.
        <br><br>

        Medellín, Antioquia - Colombia
     </h1>

     <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d1402.2208903524884!2d-75.56802179906991!3d6.253242606081857!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e442856948b8afb%3A0x2a070581fe053800!2sLa%20Candelaria%2C%20Medell%C3%ADn%2C%20La%20Candelaria%2C%20Medell%C3%ADn%2C%20Antioquia!5e0!3m2!1ses-419!2sco!4v1759869185577!5m2!1ses-419!2sco" width="200" height="200" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
     </div>

     <!-- tercera columna -->
      <div class="col small my-3 my-lg-0">
          <h4>Resuelva su Inquietud</h4>
          <form role="form" method="post">
            <div class="mb-3">
              <input type="text" class="form-control" name="nombre" placeholder="Escribe tu Nombre" required>
            </div>
            <div class="mb-3">
              <input type="email" class="form-control" name="email" placeholder="example@dominio.com" required>
            </div>
            <div class="mb-3">
              <input type="text" class="form-control" name="asunto" placeholder="Escribe el Asunto" required>
            </div>
            <div class="mb-3">
              <textarea class="form-control" name="mensaje" rows="3" placeholder="Mensaje..." required></textarea>
            </div>
            <button type="submit" class="btn btn-primary">Enviar</button>
            <br>


          </form>
        </div>  


 

    </div>
  </div>
</div>





<!-- Main Footer -->
  <footer class="main-footer">
    <!-- To the right -->
    <div class="float-right d-none d-sm-inline">
      Anything you want
    </div>
    <!-- Default to the left -->
    <strong>Copyright &copy; 2014-2021 <a href="https://adminlte.io">AdminLTE.io</a>.</strong> All rights reserved.
  </footer>